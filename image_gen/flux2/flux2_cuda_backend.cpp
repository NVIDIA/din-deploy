...
namespace
{
    ...
    public:
        ...
        void Run(Ort::SyncStream* stream, SamplingBackend backend, bool device_is_cuda, float t_curr, float t_next)
        {
            ...
            if (should_run_stage_on_cuda(backend, device_is_cuda, "EulerStage"))
            {
                ...
                const size_t hidden_count = hidden_states_->BindingValue().GetTensorTypeAndShapeInfo().
                                                            GetElementCount();
                launch_flux_euler_kernel(reinterpret_cast<cudaStream_t>(stream->GetHandle()), t_curr, t_next,
                                         hidden_count,
                       ...
                       ExecutionProviderMode provider_mode, SamplingBackend sampling_backend)
    {
        ...
        state.transformer_runner =
            std::make_unique<din::common::OrtRunner>(state.env, model_paths.transformer_model.string(), provider,
                                                     cache_dir,
                                                     ep_context,
        ...
        state.vae_decoder_runner =
            std::make_unique<din::common::OrtRunner>(state.env, model_paths.vae_decoder_model.string(), provider,
                                                     cache_dir,
                                                     ep_context,
        ...
        state.hidden_states = std::make_unique<din::common::TensorBuffer<float>>(
            *state.transformer_runner, hidden_shape,
            has_separate_binding);
        ...
        state.bn_mean =
            std::make_unique<din::common::TensorBuffer<float>>(*state.transformer_runner, bn_shape,
                                                               has_separate_binding);
        state.bn_std =
            std::make_unique<din::common::TensorBuffer<float>>(*state.transformer_runner, bn_shape,
                                                               has_separate_binding);
    ...
    Flux2Image run_initialized_pipeline(CudaPipelineState& state, unsigned int seed)
    {
...

        {
            ...
            const size_t hidden_count = state.hidden_states->BindingValue().GetTensorTypeAndShapeInfo().
                                              GetElementCount();
    ...
    public:
        ...
        void Initialize() override
        {
            ...
            const ExecutionProviderMode provider =
                config_.provider == Flux2ExecutionProvider::Cpu
                    ? ExecutionProviderMode::Cpu
                    : ExecutionProviderMode::Trt;
