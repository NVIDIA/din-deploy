...
class CppBackend:
    def __init__(
        self,
        *,
        model_dir: str | Path = Path("artifacts/nemotron/onnx"),
        provider: str = "cpu",
        ep_cache: str | Path = Path("artifacts/nemotron/trt_rtx_cache"),
        ep_context_dir: str | Path = Path("artifacts/nemotron/ep_context"),
        lang_id: str = "auto",
    ) -> None:
        try:
            from rnnt._nemotron_asr_cpp import Transcriber
        except ImportError as error:
            try:
                from _nemotron_asr_cpp import Transcriber
            except ImportError:
                raise RuntimeError(
                    "Nemotron C++ bindings are not importable. Build `_nemotron_asr_cpp` "
